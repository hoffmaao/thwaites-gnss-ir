...
d = gps.doy2ymd(year,doy); month = d.month; day = d.day
dmjd, fracS = gps.mjd(year,month,day,0,0,0)


# retrieve the inputs needed to window the data and compute Lomb Scargle Periodograms 
lat,long,ht,elval,azval,freqs,reqAmp,polyV,desiredP,Hlimits,ediff,pele,NReg = gps.read_inputs(station) 
...
# find the observation file name and try to read it
    obsfile = gps.define_filename(station,year,doy,snr_type)
    obsfile2 = gps.define_filename_prevday(station,year,doy,snr_type)
...
#   allGoodP,Psat,Pele,Pazi,Pt,Pedot,Ps1,Ps2,Ps5,Ps6,Ps7,Ps8,PsnrE = snr.read_snr_multiday(obsfile,obsfile2,twoDays)
    if (allGood == 1):
        ...
        if RefractionCorrection:
            ...
            ele = corrE
#           gps.print_file_stats(ele,sat,s1,s2,s5,s6,s7,s8,e1,e2) 
        ct = 0
# good arcs saved to a plain text file, rejected arcs to local file. Open those file names
        fout,frej = gps.open_outputfile(station,year,doy) 
# If you want to make a plot
        gps.open_plot(plt_screen)
...
#   get the list of satellites for this frequency
            if onesat == None:
                satlist = gps.find_satlist(f,snrE)
            else:
                satlist = [onesat]
...
# window the data
                    x,y,Nv,cf,UTCtime,avgAzim,avgEdot,Edot2,delT= gps.window_data(s1,s2,s5,s6,s7,s8,sat,ele,azi,t,edot,f,az1,az2,e1,e2,satNu,polyV,pele) 
#           this is a kluge for snr files that have non edot values in column 5
                    MJD = gps.getMJD(year,month,day, UTCtime)
                    if Nv > minNumPts:
                        maxF, maxAmp, eminObs, emaxObs,riseSet,px,pz= gps.strip_compute(x,y,cf,maxH,desiredP,polyV,minH) 
...
{8:6.2f} {9:4.0f} {10:3.0f} {11:2.0f} {12:8.5f} {13:6.2f} {14:7.2f} {15:12.6f} \n".format(year,doy,maxF,satNu, UTCtime,\
                       avgAzim,maxAmp,eminObs,emaxObs,Nv, f,riseSet, Edot2, maxAmp/Noise, delT, MJD))
                            gj +=1
                            gps.update_plot(plt_screen,x,y,px,pz)
                        else:
...
# plot to the screen
        gps.quick_plot(plt_screen, total_arcs,station,pltname)
